# Claude Architect가 되고 싶다면 (풀 코스)

# Claude Architect가 되고 싶다면 (풀 코스)

Anthropic이 만든 "Claude Certified Architect" 자격증 시험이 있습니다. 파트너사 소속만 응시 가능. 일반인은 못 봐요.

hoeem이라는 개발자가 시험 가이드를 통째로 뜯어서 공개했습니다.

프로덕션급 애플리케이션을 만드는 Claude Architect가 되려면 Claude Code, Claude Agent SDK, Claude API, 그리고 MCP(Model Context Protocol)를 이해해야 합니다. 이 글이 전부를 다룹니다. 아래 시험 기준으로 작성됐어요.

다만 보시다시피, "인증"을 받으려면 Claude 파트너여야 합니다. 파트너가 아니면 시험 자체를 볼 수 없어요.

**근데 그게 중요한가요?**

"Claude Certified Architect"가 되기 위한 내용을 학습할 능력이 있다면, 프로덕션급 애플리케이션을 만들 수 있습니다.

프로덕션급 애플리케이션을 만드는 데 자격증이 필요한 게 아닙니다.

지식만 있으면 됩니다.

그래서 시험 가이드 전체를 뜯어서, 진짜 중요한 것만 뽑았습니다. 당신이 Claude Architect가 될 수 있도록.

## 당신이 마주할 내용

시험은 Claude 파트너가 아니면 볼 수 없습니다. 하지만 그건 상관없어요. 이 시험을 위해 배워야 하는 내용이 아래를 가르쳐주니까요. "속았다"고 징징대지 마세요. 자격증 옆에 붙는 체크 마크 하나를 위해서가 아니라, 자기주도 학습자가 되어서 아래 내용을 **이해**함으로써 Claude Architect가 되세요:
...
- 고객 지원 해결 에이전트 (Agent SDK + MCP + 에스컬레이션)
- Claude Code로 코드 생성 (CLAUDE.md + plan 모드 + 슬래시 커맨드)
- 멀티에이전트 리서치 시스템 (코디네이터-서브에이전트 오케스트레이션)
- 개발자 생산성 도구 (빌트인 도구 + MCP 서버)
- CI/CD용 Claude Code (비대화형 파이프라인 + 구조화 출력)
- 구조화 데이터 추출 (JSON 스키마 + tool_use + 검증 루프)

---

## 도메인 1: 에이전틱 아키텍처 & 오케스트레이션 (27%)

시험은 세 가지 안티패턴을 보자마자 거부해야 합니다: 자연어를 파싱해서 루프 종료를 판단하는 것, 임의의 반복 횟수 제한을 주 정지 메커니즘으로 쓰는 것, 어시스턴트 텍스트를 완료 지표로 확인하는 것. 전부 틀려요.

가장 큰 실수: 서브에이전트가 코디네이터와 메모리를 공유한다고 가정하는 것. 공유하지 않습니다. 서브에이전트는 격리된 컨텍스트에서 동작해요. 모든 정보는 프롬프트에 명시적으로 전달해야 합니다.

가장 많은 점수를 지켜줄 규칙: 금전적이거나 보안에 민감한 작업일 때, 프롬프트 지시만으로는 충분하지 않습니다. 훅과 사전 조건 게이트로 도구 순서를 프로그래밍 방식으로 강제해야 합니다.

### 학습 자료:

- Agent SDK Overview — 에이전틱 루프 메커니즘과 서브에이전트 패턴
- Building Agents with the Claude Agent SDK — 훅, 오케스트레이션, 세션에 대한 Anthropic 공식 모범 사례
- Agent SDK Python 레포 + 예제 — 실습 코드: 훅, 커스텀 도구, fork_session

### 도메인 1 학습 프롬프트

어디서 시작할지 모르겠다면 Claude에 아래를 붙여넣으세요:
...
## 도메인 2: 도구 설계 & MCP 통합 (18%)

도구 설명(description)은 엄청나게 과소평가되고 있어요. 시험은 이걸 테스트합니다.

도구 설명은 Claude가 도구를 선택하는 핵심 메커니즘입니다. 설명이 모호하거나 겹치면 선택이 불안정해져요.

시험 샘플 문제에서 `get_customer`와 `lookup_order`의 설명이 거의 같아서 계속 잘못 라우팅되는 상황이 나옵니다. 정답은 퓨샷 예시도, 라우팅 분류기도, 도구 통합도 아니에요. 정답은 더 나은 설명을 쓰는 것.

tool_choice 옵션을 완벽히 알아야 합니다: "auto"(모델이 텍스트를 반환할 수도 있음), "any"(반드시 도구를 호출하되 어떤 것인지는 모델이 선택), 강제 선택(반드시 특정 도구를 호출). 각각 언제 쓰는지 알아야 해요.

에이전트에 18개 도구를 주면 선택 정확도가 떨어집니다. 각 서브에이전트의 역할에 맞는 4~5개로 제한하세요.

### 학습 자료:

- MCP Integration for Claude Code — 서버 스코핑, 환경변수 확장, 프로젝트/유저 설정
- MCP 사양 및 커뮤니티 서버 — 프로토콜 이해, 커뮤니티 서버 vs 커스텀 빌드 판단
- Claude Agent SDK TypeScript 레포 — 도구 정의 패턴과 구조화 에러 응답
...
## 도메인 3: CLAUDE CODE 설정 & 워크플로우 (20%)

이 도메인은 Claude Code를 "쓰는 사람"과 "팀을 위해 설정한 사람"을 구분합니다.

CLAUDE.md 계층 구조가 핵심이에요. 세 단계: 유저 레벨(~/.claude/CLAUDE.md), 프로젝트 레벨(.claude/CLAUDE.md), 디렉토리 레벨(하위 디렉토리 파일). 시험의 단골 함정: 팀원이 지침을 못 받는 이유가 유저 레벨 설정(버전 관리 안 되고 공유 안 됨)에 있기 때문.

경로별 규칙은 숨은 핵심 개념입니다. `.claude/rules/`에 YAML 프론트매터 글로브 패턴 `**/*.test.tsx`를 쓰면 전체 코드베이스에 규칙이 적용돼요. 디렉토리 레벨 CLAUDE.md로는 이게 안 됩니다. 디렉토리에 묶이니까.

Plan 모드 vs 직접 실행:

- Plan 모드: 모놀리스 리스트럭처링, 멀티 파일 마이그레이션, 아키텍처 결정
- 직접 실행: 단일 파일 버그 수정, 검증 체크 하나, 명확한 범위

스킬 프론트매터의 `context: fork`(장황한 출력을 격리), `-p` 플래그(비대화형 CI/CD), 같은 세션에서 자체 리뷰보다 독립 리뷰 인스턴스가 더 많이 잡아낸다는 것을 알아야 합니다.

### 학습 자료:

- Claude Code 공식 문서 — CLAUDE.md 계층, rules 디렉토리, 슬래시 커맨드, 스킬 프론트매터
- Claude Code CLI 치트시트 — 커맨드, 스킬, 훅, CI/CD 플래그를 하나의 실전 레퍼런스로
- Creating the Perfect CLAUDE.md — 실제 팀 설정 패턴과 MCP 통합
...
## 도메인 4: 프롬프트 엔지니어링 & 구조화 출력 (20%)

이 도메인 전체를 관통하는 두 단어: **명시적으로.**

"보수적으로 해"는 정밀도를 개선하지 않습니다. "높은 확신도만 보고해"는 오탐을 줄이지 않아요. 효과 있는 건: 어떤 이슈를 보고하고 어떤 것을 건너뛸지를 정확히 정의하고, 각 심각도 레벨에 구체적인 코드 예시를 다는 것.

퓨샷 예시가 시험에서 테스트하는 가장 효과적인 기법입니다. 2~4개의 타겟 예시, 모호한 케이스 처리 방법과 왜 한 선택을 했는지 추론을 보여주는 것.

tool_use + JSON 스키마는 구문 오류를 제거합니다. 하지만 의미 오류는 못 막아요. 스키마 설계: 소스 데이터에 없을 수 있는 필드는 nullable(조작된 값 방지), "unclear" enum 값, "other" + 상세 문자열.

Message Batches API: 50% 절감, 최대 24시간 처리, 지연 SLA 없음, 멀티턴 도구 호출 불가. 야간 보고서는 배치. 블로킹 사전 머지 체크는 동기.

### 학습 자료:

- Anthropic 프롬프트 엔지니어링 문서 — 퓨샷 패턴, 명시적 기준, 구조화 출력
- Anthropic API Tool Use 문서 — tool_use, tool_choice 설정, JSON 스키마 강제
- 시험 가이드의 샘플 문제(Q10, Q11, Q12)가 이 도메인의 최고 학습 자료입니다. 모든 오답 선택지를 분석하고 왜 틀린지 이해하세요.
...
## 도메인 5: 컨텍스트 관리 & 안정성 (15%)

비중은 가장 작지만, 여기서 실수하면 다른 모든 곳에 파급됩니다.

점진적 요약은 거래 데이터를 죽입니다. 해결: 추출된 금액, 날짜, 주문번호가 담긴 영구 "케이스 팩트" 블록. 절대 요약하지 않음. 매 프롬프트에 포함.

"중간에서 잃어버리기" 효과: 모델이 긴 입력의 중간에 묻힌 내용을 놓칩니다. 핵심 요약을 앞에 놓으세요.

세 가지 유효한 에스컬레이션 트리거: 고객이 사람을 요청(즉시 수용), 정책 공백, 진행 불가. 시험이 유혹하는 두 가지 신뢰할 수 없는 트리거: 감정 분석과 자체 보고 확신도 점수.

에러 전파를 올바르게 하는 법: 구조화된 컨텍스트(실패 유형, 시도한 쿼리, 부분 결과, 대안). 안티패턴: 에러를 조용히 무시하거나 단일 실패로 전체 워크플로우를 중단하는 것.

### 학습 자료:

- Building Agents with the Claude Agent SDK — 컨텍스트 관리, 에러 전파, 에스컬레이션 설계
- Agent SDK 세션 문서 — 재개, fork_session, /compact
- Everything Claude Code 레포 — 실전 검증된 컨텍스트 관리 패턴, 스크래치패드 파일, 전략적 컴팩션
...
이제 가서 비인증 Claude Architect가 되세요 (파트너라면 인증도 가능). 어느 쪽이든, 시작할 시간입니다.

원문 → [https://x.com/hooeem](https://x.com/hooeem/status/2033198345045336559)